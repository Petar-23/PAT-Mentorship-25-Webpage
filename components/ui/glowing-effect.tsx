...
interface GlowingEffectProps {
  ...
  proximity?: number;
  spread?: number;
  variant?: "default" | "white" | "light" | "dark" | "custom";
  glow?: boolean;
  className?: string;
  disabled?: boolean;
  movementDuration?: number;
  borderWidth?: number;
  color?: string; // Hex color for custom variant
}
  ...
  ({
    ...
    return (
      <>
        <div
          className={cn(
            "pointer-events-none absolute -inset-px hidden rounded-[inherit] border border-slate-200 opacity-0 transition-opacity dark:border-slate-800",
            glow && "opacity-100",
          ...
          style={
            {
              ...
              "--gradient":
                ...
                repeating-conic-gradient(
                  ...
                  ? `radial-gradient(circle, ${color} 10%, ${color}00 20%),
                radial-gradient(circle at 40% 40%, ${color} 5%, ${color}00 15%),
                radial-gradient(circle at 60% 60%, ${color} 10%, ${color}00 20%), 
                radial-gradient(circle at 40% 60%, ${color} 10%, ${color}00 20%),
                repeating-conic-gradient(
                  from 236.84deg at 50% 50%,
                  ${color} 0%,
                  ${color} calc(25% / var(--repeating-conic-gradient-times)),
                  ${color} calc(50% / var(--repeating-conic-gradient-times)), 
                  ${color} calc(75% / var(--repeating-conic-gradient-times)),
                  ${color} calc(100% / var(--repeating-conic-gradient-times))
                )`
