...
type PayPalAdminClientProps = {
  initialSubscribers: PayPalSubscriber[]
}

export default function PayPalAdminClient({ initialSubscribers }: PayPalAdminClientProps) {
  const [ids, setIds] = useState('')
  const [loading, setLoading] = useState(false)
  const [result, setResult] = useState<ImportResult | null>(null)
  const [error, setError] = useState<string | null>(null)

  const [subscribers, setSubscribers] = useState<PayPalSubscriber[]>(initialSubscribers)
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-neutral-600">
          ...
          <p className="mt-2 text-xs text-neutral-400">
            Alternativ: PayPal Dashboard &rarr; Berichte &rarr; Alle Transaktionen &rarr;
            Subscription-Transaktionen filtern &rarr; Subscription-ID aus den Details
            kopieren.
          </p>
        </CardContent>
      </Card>
