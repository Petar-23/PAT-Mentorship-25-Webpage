...
function fixture(change = {}) {
  const channel = { id: TARGET.channelId, guild_id: TARGET.guildId, type: 0, permission_overwrites: [{id:TARGET.guildId,type:0,deny:'1024',allow:'0'},{id:TARGET.memberRoleId,type:0,deny:'0',allow:'1024'},{id:'botrole',type:0,deny:'0',allow:'248832'}], ...change }
  return async path => {
...
test('requires channel-effective everyone permission and reports the future policy',async()=>{
  const allowed=await verifyTarget(env,fixture())
  assert.equal(allowed.mentionPolicy,'everyone-only');assert.equal(allowed.canMentionEveryone,true)
  const denied=fixture({permission_overwrites:[{id:TARGET.guildId,type:0,deny:'1024',allow:'0'},{id:TARGET.memberRoleId,type:0,deny:'0',allow:'1024'},{id:'botrole',type:0,deny:'0',allow:'117760'}]})
  await assert.rejects(verifyTarget(env,denied),/MENTION_EVERYONE_MISSING/)
})
...
test('multipart sends let fetch set the boundary while JSON keeps its content type',async()=>{
  const seen=[];const api=discordApi(env,async(url,init)=>{seen.push(init);return Response.json({id:'message'})})
  const body=new FormData();body.append('payload_json','{}')
  await api('/test',{method:'POST',body});await api('/test',{method:'POST',body:'{}'})
  assert.equal(seen[0].headers['Content-Type'],undefined);assert.equal(seen[0].body,body)
  assert.equal(seen[1].headers['Content-Type'],'application/json')
})
test('thumbnail attachment rights are mandatory',async()=>{
  await assert.rejects(verifyTarget(env,fixture({permission_overwrites:[{id:'botrole',type:0,deny:'32768',allow:'216064'}]})),/PERMISSIONS_MISSING/)
})
test('member-specific mention denial overrides role allow',async()=>{
  const denied=fixture({permission_overwrites:[{id:'botrole',type:0,deny:'0',allow:'248832'},{id:'bot',type:1,deny:'131072',allow:'0'}]})
  await assert.rejects(verifyTarget(env,denied),/MENTION_EVERYONE_MISSING/)
})
