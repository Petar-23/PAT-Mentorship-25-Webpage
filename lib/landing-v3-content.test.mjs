...
// Landingpage (Startseite, vorher /lp-v3): Regeln für Texte und Einbindung, die sonst nur beim Lesen auffallen würden.
...
const files = [...LANDING_DIRS.flatMap(listFiles), join(ROOT, 'app/page.tsx')].filter((path) => /\.(tsx?|css|mjs)$/.test(path))
...
test('Landingpage: ist die indexierbare Startseite, /lp-v3 leitet dauerhaft dorthin', () => {
  const page = read(join(ROOT, 'app/page.tsx'))
  assert.match(page, /<LandingV3 /)
  assert.doesNotMatch(page, /index: false/)
  assert.match(page, /canonical: '\/'/)
  const old = read(join(ROOT, 'app/lp-v3/page.tsx'))
  assert.match(old, /permanentRedirect\('\/'\)/)
  const navbar = read(join(ROOT, 'components/layout/navbar.tsx'))
  assert.match(navbar, /hasOwnHeader = isMentorship \|\| pathname === '\/'/)
