...
// Antwortet Gmail mit 401 (gespeichertes Token abgelaufen oder widerrufen), wird einmal mit frischem
// Token wiederholt, innerhalb derselben Zeitgrenze.
...
/**
 * Eine einzelne, reine ASCII-Adresse ohne Anzeigename (dieselbe Prüfung wie beim Versand).
 * @param {unknown} value
 */
...
export function createGmailSender(config) {
  ...
  /**
   * Verwirft das Token nur, wenn es noch das gespeicherte ist. Sonst würde ein später
   * eintreffendes 401 eines alten Tokens ein inzwischen frisch geholtes wegwerfen.
   * @param {string} token
   */
   ...
   * @param {AbortSignal} signal
   */
  async function postMessage(token, raw, signal) {
    try {
      return await config.fetch(GMAIL_SEND_URL, {
        method: 'POST',
  ...
  /**
   * Wirft MailError. Eine Zeitgrenze gilt für Token und Versand zusammen (auch für die Wiederholung).
  ...
  async function send(mail, options = {}) {
    ...
    let res = await postMessage(token, raw, signal)
    if (res.status === 401) {
      // Gespeichertes Token abgelaufen oder widerrufen. 401 heißt, Gmail hat die Mail nicht
      // angenommen, deshalb einmal mit frischem Token wiederholen (keine doppelte Zustellung).
      dropToken(token)
      await res.text().catch(() => '')
      token = await accessToken(signal)
      res = await postMessage(token, raw, signal)
      if (res.status === 401) dropToken(token)
    }
    if (!res.ok) {
