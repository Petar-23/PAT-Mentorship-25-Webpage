...
function renderConversion(previousSubscription, { guestCheckout = false } = {}) {
  const exports = {}
  const deps = {
    react: React,
    'react/jsx-runtime': jsxRuntime,
    'next/navigation': { useRouter: () => ({ refresh: () => {} }) },
    'next/link': { default: ({ href, prefetch, children }) => React.createElement('a', { href, 'data-prefetch': String(prefetch) }, children) },
...
test('guest checkout switched on: the booking button leads to /checkout instead of the old consent box', () => {
  const html = renderConversion(null, { guestCheckout: true })
  assert.match(html, /<a href="\/checkout\?src=dashboard" data-prefetch="false">Weiter zur Buchung<\/a>/)
  assert.doesNotMatch(html, /data-checkout|id="terms"/)
})

test('guest checkout switched off: the old consent box and checkout button stay', () => {
  const html = renderConversion(null, { guestCheckout: false })
  assert.match(html, /id="terms"/)
  assert.match(html, /data-checkout/)
  assert.doesNotMatch(html, /\/checkout\?src=/)
})
