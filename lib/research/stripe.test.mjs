...
test('portal: uses the research portal configuration when set', async () => {
  const rows = [{ userId: USER, stripeCustomerId: 'cus_cached', stripeSubscriptionId: 'sub_1', status: 'active' }]
  await withEnv({ STRIPE_RESEARCH_PORTAL_CONFIGURATION_ID: 'bpc_research' }, async () => {
    const s = scenario({ rows })
    assert.deepEqual(await s.research.createResearchPortalSession({ userId: USER, returnUrl: 'https://research.example.test/account' }), {
      url: 'https://billing.stripe.com/p/session/test_123',
    })
    assert.deepEqual(s.calls, [
      ['billingPortal.sessions.create', { customer: 'cus_cached', return_url: 'https://research.example.test/account', locale: 'en', configuration: 'bpc_research' }],
    ])
  })
  await withEnv({}, async () => {
    const s = scenario({ rows })
    await s.research.createResearchPortalSession({ userId: USER, returnUrl: 'https://research.example.test/account' })
    assert.equal('configuration' in s.calls[0][1], false)
  })
})
...
test('setup script: --apply creates product, prices and portal once; a re-run changes nothing', async () => {
  ...
  assert.deepEqual(created.map(([name]) => name), ['products.create', ...Array(6).fill('prices.create'), 'configurations.create'])
  assert.deepEqual(created[0][1].metadata, { pat_product: 'research' })
  assert.deepEqual(created[0][2], { idempotencyKey: 'pat-research-setup-v1-product' })
  for (const [, params] of created.slice(1, 7)) {
  ...
  const portal = created[7][1]
  assert.deepEqual(portal.features.subscription_update.products, [{ product: first.productId, prices: Object.values(first.priceIds) }])
  assert.equal(portal.features.subscription_update.proration_behavior, 'none')
  ...
  assert.deepEqual(first.envLines, [
    `STRIPE_RESEARCH_PRODUCT_ID=${first.productId}`,
    ...desiredResearchPrices().map(p => `${p.envName}=${first.priceIds[p.envName]}`),
    `STRIPE_RESEARCH_PORTAL_CONFIGURATION_ID=${first.portalConfigurationId}`,
  ])
...
test('setup script: existing prices that differ abort without any write', async () => {
  const product = { id: 'prod_research', active: true, metadata: { pat_product: 'research' } }
  const wrong = {
    id: 'price_wrong',
    active: true,
    currency: 'usd',
...
test('setup script: lookup-key prices without the marked product abort before the product is created', async () => {
  ...
  await assert.rejects(
    ...
    /nothing was changed.*no product with pat_product=research was found/s
  )
  assert.deepEqual(writes(fake.calls), [])
  assert.equal(fake.products.length, 1)

  // Auch eine doppelte eigene Portal-Konfiguration bricht vor jedem Schreiben ab.
  const marker = { pat_product: 'research' }
  ...
  await assert.rejects(runResearchStripeSetup({ stripe: twoPortals.stripe, args: setupArgs({ apply: true }), log: () => {} }), /2 active research portal configurations/)
  assert.deepEqual(writes(twoPortals.calls), [])
})

test('setup script: ambiguous or archived research products abort', async () => {
  const marker = { pat_product: 'research' }
  const twice = fakeSetupStripe({ products: [{ id: 'prod_a', active: true, metadata: marker }, { id: 'prod_b', active: true, metadata: marker }] })
  await assert.rejects(runResearchStripeSetup({ stripe: twice.stripe, args: setupArgs({ apply: true }), log: () => {} }), /Found 2 products/)
  const archived = fakeSetupStripe({ products: [{ id: 'prod_a', active: false, metadata: marker }] })
  await assert.rejects(runResearchStripeSetup({ stripe: archived.stripe, args: setupArgs({ apply: true }), log: () => {} }), /archived/)
  assert.deepEqual([...writes(twice.calls), ...writes(archived.calls)], [])
})

test('setup script: drift in the own portal configuration is reported and fixed only with --apply', async () => {
  const fake = fakeSetupStripe({ products: [{ id: 'prod_mentorship', active: true, metadata: {} }] })
  const first = await runResearchStripeSetup({ stripe: fake.stripe, args: setupArgs({ apply: true }), log: () => {} })
  const own = fake.configurations.find(c => c.id === first.portalConfigurationId)
  own.features = { ...own.features, subscription_update: { ...own.features.subscription_update, proration_behavior: 'always_invoice' } }

  const dryLines = []
  const before = fake.calls.length
  await runResearchStripeSetup({ stripe: fake.stripe, args: setupArgs({ apply: false }), log: line => dryLines.push(line) })
  assert.deepEqual(writes(fake.calls.slice(before)), [])
  assert.ok(dryLines.some(line => line.includes('would update portal configuration') && line.includes('proration_behavior always_invoice')))
  ...
  assert.deepEqual(updates.map(([name, id]) => [name, id]), [['configurations.update', first.portalConfigurationId]])
  assert.equal(updates[0][2].features.subscription_update.proration_behavior, 'none')
})

test('setup script: portal drift detection', () => {
  const desired = buildPortalConfigurationParams({ productId: 'prod_r', priceIds: ['p1', 'p2'], termsUrl: null, privacyUrl: null })
  const matching = { ...desired, features: { ...desired.features, subscription_update: { ...desired.features.subscription_update, products: [{ product: 'prod_r', prices: ['p2', 'p1'] }] } } }
