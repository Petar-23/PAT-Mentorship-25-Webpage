 ...
 * @returns {{ message: string, action: 'account' | 'sign-in' | null }}
 */
export function researchCheckoutError(code, serverMessage) {
  switch (code) {
    ...
    case 'rate_limited':
      return { message: 'Too many checkout attempts. Please wait a few minutes and try again.', action: null }
    case 'signed_out':
 ...
 * @param {unknown} code
 */
export function researchPortalError(status, code) {
  if (status === 404 || code === 'no_customer') return 'No subscription found yet.'
  if (status === 401 || code === 'signed_out') return 'Your session has ended. Please sign in again.'
  if (status === 429 || code === 'rate_limited') return 'Too many attempts. Please wait a few minutes and try again.'
