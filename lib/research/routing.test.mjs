...
// clerkMiddleware setzt diesen Header auf jede Anfrage, die sie verarbeitet hat.
const CLERK_MIDDLEWARE_HEADERS = { 'x-clerk-auth-status': 'signed-out' }

function loadRequestContext(headerValues = {}, { middleware = true } = {}) {
  return loadTs('./request-context.ts', {
    'server-only': {},
    react: { cache: (fn) => fn },
    'next/headers': { headers: async () => new Headers({ ...(middleware ? CLERK_MIDDLEWARE_HEADERS : {}), ...headerValues }) },
    'next/navigation': {
      notFound: () => {
...
// Der Matcher überspringt Pfade mit Datei-Endung auch auf erlaubten Hosts: Dann
// lief clerkMiddleware nie, Clerk auth() würde werfen (500) — stattdessen 404.
test('getResearchRequestContext calls notFound() when the request skipped clerkMiddleware', async () => {
  // @clerk/backend ist keine direkte Abhängigkeit: über @clerk/nextjs auflösen, also
  // genau die Kopie, die dessen clerkMiddleware und auth() verwenden.
  const clerkNextjsServer = createRequire(import.meta.url).resolve('@clerk/nextjs/server')
  const clerk = createRequire(clerkNextjsServer)('@clerk/backend/internal')
  // Stolperdraht: Clerk erkennt seine Middleware an genau diesem Header.
  assert.equal(clerk.constants.Headers.AuthStatus, 'x-clerk-auth-status')

  await withEnv(PRODUCTION_PROCESS_ENV, async () => {
    const skipped = loadRequestContext({ host: RESEARCH_HOST, 'x-forwarded-proto': 'https' }, { middleware: false })
    await assert.rejects(skipped.getResearchRequestContext(), NotFoundSignal)
    const empty = loadRequestContext({ host: RESEARCH_HOST, 'x-clerk-auth-status': '' }, { middleware: false })
    await assert.rejects(empty.getResearchRequestContext(), NotFoundSignal)
    for (const status of Object.values(clerk.AuthStatus)) {
      const passed = loadRequestContext({ host: RESEARCH_HOST, 'x-clerk-auth-status': status }, { middleware: false })
      assert.equal((await passed.getResearchRequestContext()).isResearchHost, true, status)
    }
  })
  await withEnv({ NODE_ENV: 'development', VERCEL_ENV: undefined, RESEARCH_PUBLIC_HOST: undefined, RESEARCH_EXTRA_HOSTS: undefined }, async () => {
    await assert.rejects(loadRequestContext({ host: 'localhost:3000' }, { middleware: false }).getResearchRequestContext(), NotFoundSignal)
  })
})
...
function loadSurface() {
  const state = { pathname: '/', hydrating: false }
  const fakeReact = {
    useSyncExternalStore: (subscribe, getSnapshot, getServerSnapshot) => {
      assert.equal(typeof subscribe(() => {}), 'function')
      return state.hydrating ? getServerSnapshot() : getSnapshot()
  ...
  const hooks = loadTs('../../components/research/surface.tsx', {
    react: fakeReact,
    'next/navigation': { usePathname: () => state.pathname },
    '@/lib/research/surface.mjs': surface,
  })
  return { hooks, state }
}

test('the tiny root-chrome surface module agrees with the routing module', () => {
  for (const hostname of [RESEARCH_HOST, 'research-staging.price-action-trader.de', 'research.localhost', 'RESEARCH.price-action-trader.de', MAIN_HOST, 'localhost', '', null, undefined, 'research.', 'myresearch.example.com']) {
    assert.equal(surface.isResearchSurfaceHostname(hostname), isResearchBrowserHostname(hostname), String(hostname))
  }
  for (const pathname of ['/research', '/research/', '/research/pricing', '/researcher', '/', '/mentorship', '', null]) {
    assert.equal(surface.isResearchSurfacePathname(pathname), isResearchPathname(pathname), String(pathname))
  }
  const source = fs.readFileSync(new URL('./surface.mjs', import.meta.url), 'utf8')
  assert.ok(!/^\s*import\s/m.test(source), 'surface.mjs must stay import-free (it ships in every page bundle)')
})

test('root chrome treats research as unknown until the browser host is known', async () => {
  const { hooks, state } = loadSurface()
  const cases = [
