...
import { RESEARCH_PRODUCT, RESEARCH_PRODUCT_ID_ENV, isResearchInterval, isResearchTier, resolveResearchPrice } from './config.mjs'
 ...
 * 2. Fallback für rotierte Preise: Preis gehört zum Research-Produkt
 *    (STRIPE_RESEARCH_PRODUCT_ID) und trägt gültige Metadaten
 *    research_tier/research_interval.
 *
...
export function resolveResearchSubscriptionPrice(subscription, env) {
  ...
  const researchProductId = env[RESEARCH_PRODUCT_ID_ENV]?.trim()
  const productId = idOf(price.product)
  if (!researchProductId || !productId || productId !== researchProductId) return null
