...
test('rotated prices fall back to metadata only on the product of exactly that tier', () => {
  const rotated = { id: 'price_rotated', product: 'prod_supporter', metadata: { research_tier: 'supporter', research_interval: 'year' } }
  ...
  assert.deepEqual(
    resolveResearchSubscriptionPrice(subscription({ price: { ...rotated, product: { id: 'prod_supporter', name: 'PAT Research Supporter' } } }), ENV),
    { priceId: 'price_rotated', tier: 'supporter', interval: 'year' }
  )
})

test('unknown prices never resolve: foreign or wrong-tier product, invalid metadata, missing product env, missing price', () => {
  ...
  const unknown = [
    ...
    // Metadaten behaupten "member", der Preis hängt aber am Reader-Produkt
    subscription({ price: { id: 'price_x', product: 'prod_reader', metadata: meta } }),
    subscription({ price: { id: 'price_x', product: 'prod_member', metadata: { research_tier: 'gold', research_interval: 'month' } } }),
    subscription({ price: { id: 'price_x', product: 'prod_member', metadata: { research_tier: 'member', research_interval: 'week' } } }),
    subscription({ price: { id: 'price_x', product: 'prod_member', metadata: null } }),
  ...
  const withoutProductEnv = { ...ENV, STRIPE_RESEARCH_PRODUCT_ID_MEMBER: undefined }
  assert.equal(
    resolveResearchSubscriptionPrice(subscription({ price: { id: 'price_x', product: 'prod_member', metadata: meta } }), withoutProductEnv),
    null
  )
  assert.equal(
    resolveResearchSubscriptionPrice(subscription({ price: { id: 'price_x', product: '', metadata: meta } }), { ...ENV, STRIPE_RESEARCH_PRODUCT_ID_MEMBER: '' }),
    null
  )
  // Die frühere Einzelprodukt-Variable gibt keinen Fallback mehr
  assert.equal(
    resolveResearchSubscriptionPrice(subscription({ price: { id: 'price_x', product: 'prod_research', metadata: meta } }), { ...ENV, STRIPE_RESEARCH_PRODUCT_ID: 'prod_research' }),
    null
  )
})
