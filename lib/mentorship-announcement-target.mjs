...
export const REVISION = 'mentorship-target-v1'
const VIEW = 1024n, SEND = 2048n, EMBED = 16384n, HISTORY = 65536n, ADMIN = 8n
...
export async function verifyTarget(env, api) {
  ...
  const required = VIEW | SEND | EMBED | HISTORY
  ...
  return { ...target, botId: bot.id, guildName: guild.name, channelName: channel.name, revision: REVISION }
}
...
export function discordApi(env, fetcher = fetch) {
  return async (path, init = {}) => {
    let response
    try {
      response = await fetcher(`https://discord.com/api/v10${path}`, { ...init, headers: { Authorization: `Bot ${env.DISCORD_BOT_TOKEN}`, 'Content-Type': 'application/json' }, cache: 'no-store', signal: AbortSignal.timeout(10000) })
